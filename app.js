...
app.get("/states/", async (request, response) => {
  ...
  response.send(
    statesArray.map((eachStates) => {
      return convertStateDbObjectToResponseObject(eachStates);
    })
  );
});
...
app.get("/districts/", async (request, response) => {
  const getAllDistrictQuery = `
    ...
    FROM
      district;`;
  const districtsAllArray = await database.all(getAllDistrictQuery);
  response.send(
    districtsAllArray.map((eachDist) => {
      return convertDistrictDbObjectToResponseObject(eachDist);
    })
  );
});
